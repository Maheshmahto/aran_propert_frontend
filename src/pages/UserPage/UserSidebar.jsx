  ...
  useEffect(() => {
    ...
    if (selectedCity && selectedCity !== "") {
      filtered = filtered.filter(
        (property) =>
          property.city_name?.toLowerCase() === selectedCity.toLowerCase()
      );
    }
    ...
    // Filter by Buy rate (rate_buy) if "Any Price" is not selected
    if (!anyPrice) {
      filtered = filtered.filter(
        (property) =>
          property.rate_buy >= priceRange[0] && property.rate_buy <= priceRange[1]
      );
    }

    // Update the filtered properties in the parent component
    setFilteredPropertiesSidebar(filtered);
  }, [selectedCity, selectedPropertyTypes, priceRange, anyPrice, properties]);

  // Handle "Any Price" checkbox change
